...
function ajoutPeriode($dateDebut = '', $ligne_id = '', $periode_id = '', $heureDebut = '') {
	...
	for ($i = 0; $i < 24; $i++) {
		if ($i<12){$ampm="am";$hour=$i;}else {$ampm="pm";$hour=$i-12;}
		$heure1="$i:00";
		$heure2=($i+1).":00";
		...
		if (!in_array($i,$tabTranchesHoraires)){
			...
			{
				$max_time=$hour2;
				if (empty($min_time)) $min_time=$hour1;
			}
	}
	$heures_exclues="";
	...
	$objResponse->addScript('jQuery("#myModal").modal("hide")');
	$objResponse->addScript('jQuery("#myModal .modal-header h5").html("' . addslashes($smarty->getConfigVars('ajax_titreNouvellePeriode')) . '")');
	$objResponse->addScript('jQuery("#myModal .modal-body").html("' . xajaxFormat($smarty->getHtml('periode_form_new.tpl')) . '")');
...
function supprimerStatus($status_id) {
	...
	// on remplace ce statut par le premier statut par defaut qu'on trouve
	if(in_array($status->affichage, array('t', 'tp'))){
		$check = new GCollection('Status');
		$check->db_load(array('affichage', 'IN', array('t', 'tp'), 'status_id', '<>', $status->status_id), array('defaut' => 'DESC', 'priorite' => 'ASC'));
		if($check->getCount() == 0){
			$objResponse->addAlert(addslashes($smarty->getConfigVars('erreur')));
			$objResponse->addScript('location.reload();');			
		}
		$new = $check->fetch();
		$sql = 'UPDATE planning_periode SET statut_tache = ' . val2sql($new->status_id) . ' WHERE statut_tache = ' . val2sql($status->status_id);
		db_query($sql);
	}

	// on remplace ce statut par le premier statut par defaut qu'on trouve
	if(in_array($status->affichage, array('p', 'tp'))){
		$check = new GCollection('Status');
		$check->db_load(array('affichage', 'IN', array('p', 'tp'), 'status_id', '<>', $status->status_id), array('defaut' => 'DESC', 'priorite' => 'ASC'));
		if($check->getCount() == 0){
			$objResponse->addAlert(addslashes($smarty->getConfigVars('erreur')));
			$objResponse->addScript('location.reload();');			
		}
		$new = $check->fetch();
		$sql = 'UPDATE planning_projet SET statut = ' . val2sql($new->status_id) . ' WHERE statut = ' . val2sql($status->status_id);
		db_query($sql);
	}
